   ...
   stages {
      stage('Build') {
         ...
         steps {
            ...
            git branch:"${BRANCH_NAME}", url:'https://github.com/vitaliisotnichenko/docler_test_task_python'
      ...
      stage("high_priority")  {
          steps{
              //Run only tests with high priority

              sh '''
                    /Library/Frameworks/Python.framework/Versions/3.7/bin/python3 -m venv venv
                    . venv/bin/activate
                    python3 -m pytest -m high -v
                 '''
